...
BEGIN
    ...
    DECLARE livros_cursor CURSOR FOR 
    SELECT nome_genero FROM Genero WHERE nome_genero = nome_genero_param;
    ...
    read_loop: LOOP
        FETCH livros_cursor INTO livro_genero;
        IF done THEN
            LEAVE read_loop;
        END IF;

        SELECT COUNT(*) INTO total_livros FROM Livro WHERE id_genero = (SELECT id FROM Genero WHERE nome_genero = livro_genero);
    END LOOP;
...
BEGIN
    ...
    DECLARE livros_autor_cursor CURSOR FOR 
    SELECT id_livro FROM Livro_Autor WHERE id_autor = (SELECT id FROM Autor WHERE primeiro_nome = primeiro_nome_param AND ultimo_nome = ultimo_nome_param);
    
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = 1;

    OPEN livros_autor_cursor;

    read_loop: LOOP
        FETCH livros_autor_cursor INTO id_livro;
        IF done THEN
            LEAVE read_loop;
        END IF;

        SELECT titulo INTO lista_livros FROM Livro WHERE id = id_livro;

        SET lista_livros = CONCAT(lista_livros, titulo, ', ');
    END LOOP;

    CLOSE livros_autor_cursor;
...
BEGIN
    ...
    update_loop: LOOP
        FETCH livros_cursor INTO id_livro, resumo_atual;
        IF done THEN
            LEAVE update_loop;
        END IF;
        ...
        UPDATE Livro SET resumo = resumo_atual WHERE id = id_livro;
    END LOOP;
...
BEGIN
    ...
    DECLARE editoras_cursor CURSOR FOR 
    SELECT id FROM Editora;

    DECLARE livros_cursor CURSOR FOR 
    SELECT COUNT(*) FROM Livro WHERE id_editora = editora_id;

    DECLARE CONTINUE HANDLER FOR NOT FOUND SET total_editoras = -1;

    OPEN editoras_cursor;

    editoras_loop: LOOP
        FETCH editoras_cursor INTO editora_id;
        ...
        FETCH livros_cursor INTO temp_count;
        SET total_livros = total_livros + temp_count;

        CLOSE livros_cursor;
    END LOOP;

    CLOSE editoras_cursor;
...
BEGIN
    ...
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET autor_id = -1;

    OPEN autores_cursor;

    autores_loop: LOOP
        FETCH autores_cursor INTO autor_id, autor_nome;
        IF autor_id = -1 THEN
            LEAVE autores_loop;
        END IF;
